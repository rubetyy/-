        ...
        <h2>채팅</h2>
          <li v-for='chatroom in myPageInfo.chatlist' :key='chatroom.chatroompk'>
            <span v-if='userId==chatroom.useridseller'>
              <router-link :to="{name: 'ChatRoom', params: {pk: chatroom.chatroompk, nickname: chatroom.buyernickname }}">{{chatroom.buyernickname}}</router-link>
            </span>
            <span v-else>
              <router-link :to="{name: 'ChatRoom', params: {pk: chatroom.chatroompk, nickname: chatroom.sellernickname }}">{{chatroom.sellernickname}}</router-link>
            </span>
          </li>
      </ul>
    </div>
